    ...
    global.knexCon("m_level").select("m_level.*", "m_game.game_name").select(global.knexCon.raw(`group_concat(question_id) as question_id`)).join("m_game", "m_level.game_id", "=", "m_game.game_id")
        .leftJoin("m_level_link_question", "m_level.level_id", "=", "m_level_link_question.level_id").where((builder) => {
            ...
            if (response.data.length == 1) {
                global.knexCon("m_questions").whereIn("question_id", response.data[0].question_id).orderByRaw('RAND()').then(res33 => {
                    res33.map(y => {
                        y["UserAnswer"] = ""
                    })
...
module.exports.addScore = async (req, res) => {
    ...
    if (responseError.status) {
        ...
        let LevelArray = await global.knexCon("m_level").where({ level_id: reqbody.level_id });
        ...
        if (passedStatus) {
            nextLevelId = await global.knexCon("m_level").where({ level: LevelArray[0].level + 1, game_id: LevelArray[0].game_id })
            nextLevelId = nextLevelId[0].level + "-" + nextLevelId[0].level_id
        }

        let percentageCalculate = (parseInt(reqbody.level_score) / parseInt(reqbody.level_out_of)) * 100;
            ...
            if (response.length > 0) {
                ...
                if (checkExamForLevel.length == 0) {
                    global.knexCon("m_user").update({ user_points: updatePoints }).where({ user_id: reqbody.user_id }).then(resUser => {
                        res.send({
                            status: true, "Record": obj, msg: msg, userArray: UserArray, passedStatus: passedStatus, starCheck: sendArrayStar, nextLevel: nextLevelId
                        })
...
module.exports.getLevelByGame = async (req, res) => {
    let game_level = await global.knexCon.raw(`select m_level.*,level_score,if(level_score is null,0,(level_score/level_out_of)*100) as percentage from m_level left join user_level_score on user_level_score.level_id=m_level.level_id and user_level_score.score_is_active='1' and user_level_score.user_id="${req.query.user_id}" where game_id=${req.query.game_id}
    `);

    let user_level = await global.knexCon("user_level_score").select("m_level.level").join("m_level", "m_level.level_id", "=", "user_level_score.level_id").where({ "user_level_score.user_id": req.query.user_id, "score_is_active": "1", game_id: req.query.game_id }).orderBy(" m_level.level", "DESC").limit(1);
    ...
    res.send({
        status: true, "Record": game_level[0], msg: "Game Level", current_user_level: user_level.length > 0 ? user_level[0].level + 1 : 1
    })
